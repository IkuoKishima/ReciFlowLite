...
final class IngredientEngineStore: ObservableObject {
    ...
    func markDirtyAndScheduleSave(reason: String = "") {
        ...
        if !reason.isEmpty { DBLOG("🟨 markDirty: \(reason)") }
    ...
    func flushSave(reason: String = "") {
        ...
        if !reason.isEmpty { DBLOG("🟧 flushSave: \(reason)") }
...
DBLOG("✅ saved \(rows.count) rows (force=\(force))")
    ...
    func loadIfNeeded() {
        ...
        DBLOG("🟦 loadIfNeeded start recipeId=\(parentRecipeId)")


        // ✅ 先に rows を見て、必要なときだけロックする（安全）
        if !rows.isEmpty {

            DBLOG("🟦 loadIfNeeded early return (rows already exist) count=\(rows.count)")
        ...
        Task {
            ...
            // ✅ ここで1回だけ取得（async版）
            let loaded = await DatabaseManager.shared.fetchIngredientRows(
            ...
            await MainActor.run {
                ...
                DBLOG("🟦 fetchIngredientRows end count=\(loaded.count)")
                ...
                DBLOG("🟦 first seed: one empty single row")
...
extension IngredientEngineStore {
    ...
    @discardableResult
    func addSingleAtGlobalRail() -> UUID {
    ...
    @discardableResult
    func addBlockHeaderAtGlobalRail() -> UUID {
